package com.ayantsoft.expm.filter;
...
import com.ayantsoft.expm.jsf.view.LoginBean;
...
public class LoginPhaseListener implements PhaseListener {
	 ...
	 * serialVersionUID
	 */
	private static final long serialVersionUID = -2435480794209658326L;
	...
	public void afterPhase(PhaseEvent event) {
		...
		if(loginBean == null){
			...
			if(!facesContext.getViewRoot().getViewId().equals("/login.xhtml")){
				if(loginBean.getUser() == null || loginBean.getLoginCheck() == false){
					ConfigurableNavigationHandler configurableNavigationHandler = (ConfigurableNavigationHandler) FacesContext.getCurrentInstance().getApplication().getNavigationHandler();
					configurableNavigationHandler.performNavigation("/login.xhtml");
				}
			}
